...
class BlockManager:
    def __init__(self, num_blocks: int, block_size: int, enable_prefix_caching: bool = True, deterministic: bool = False):
    ...
    def can_allocate(self, seq: Sequence) -> int:
        ...
        if self.deterministic:
            # only share blocks strictly before the one holding the last prompt token: their kv was written by
            # canonical prefill chunks, and the final prompt chunk has to run to produce the first token
            max_cached_blocks = (seq.num_prompt_tokens - 1) // self.block_size
        else:
            max_cached_blocks = seq.num_blocks - 1
    ...
    def rollback(self, seq: Sequence):
        ...
        while len(seq.block_table) > seq.num_blocks:
            ...
            if block.ref_count == 0:
                self._deallocate_block(block_id)
        # the boundary block holds the correction token, which is a generated position. in deterministic mode
        # only full prompt blocks are ever hashed/shared, so this block is always private to seq
    ...
    def hash_blocks(self, seq: Sequence):
        ...
        if self.deterministic:
            # generated tokens get their kv from the verifier, not from prefill; sharing it would let a
            # cache hit produce different bits than recomputing the same tokens
            end = min(end, seq.num_prompt_tokens // self.block_size)
        if start >= end: return
